...
public class HVGALayoutParameters implements LayoutParameters {
    private static final String TAG = HVGALayoutParameters.class.getSimpleName();
    ...
    public HVGALayoutParameters(Context context, int type) {
        ...
        if (LOCAL_LOGV) {
            Log.d(TAG, "HVGALayoutParameters.<init>(" + type + ").");
        }
        mType = type;
        ...
        if (LOCAL_LOGV) {
            Log.d(TAG, "HVGALayoutParameters mMaxWidth: " + mMaxWidth +
